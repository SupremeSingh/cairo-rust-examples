use algorithms_search::binary_search::binary_search;
...
fn value_not_found_length_one() {
    ...
    let span = arr.span();
    assert(binary_search(span, 50).is_none() == true, 'non existent value was found')
}
...
fn zero_length_span() {
    ...
    let span = arr.span();
    assert(binary_search(span, 100).is_none() == true, 'non existent value was found')
}
